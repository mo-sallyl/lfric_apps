...
REAL(KIND=real_umphys), INTENT(IN) ::                                          &
 topography (tdims_s%i_start:tdims_s%i_end,                                    &
             tdims_s%j_start:tdims_s%j_end),                                   &
 sigma_h    (tdims_s%i_start:tdims_s%i_end,                                    &
             tdims_s%j_start:tdims_s%j_end),                                   &
 landfrac   (tdims_s%i_start:tdims_s%i_end,                                    &
             tdims_s%j_start:tdims_s%j_end),                                   &
 horiz_scale(tdims_s%i_start:tdims_s%i_end,                                    &
             tdims_s%j_start:tdims_s%j_end)
...
REAL(KIND=4), DIMENSION(1,n_nodes_0)         :: tqp_in_nn
REAL(KIND=4), DIMENSION(1,3*nn_nz)           :: cloud_out_nn
...
LOGICAL, SAVE :: ennuf_printed = .FALSE.
...
IF (.NOT. ennuf_printed) THEN
  PRINT *, 'ENNUF_CLD: entered; calling crmml_ennuf with 215 inputs'
  ennuf_printed = .TRUE.
END IF
...
!$OMP PARALLEL                                                                 &
!$OMP DEFAULT(NONE)                                                            &
!$OMP PRIVATE(i,j,k,temp_1d,q_1d,pressure_1d,tqp_in,cloud_out,                 &
  ...
  DO j = tdims%j_start,tdims%j_end
    DO i = tdims%i_start,tdims%i_end
      ...
      ELSE
        ...
        END DO
        ! Paste in the scalar ancillary information (with 1st rescaling).
        ...
        ! Note we are using rho=1 and g=9.81
      ...
      DO k = 1, n_nodes_0
        tqp_in_nn(1,k) = REAL(tqp_in(k), KIND=4)
      END DO

      ! ENNUF execution marker: when this subroutine is called, this NN inference
      ! is the active prediction step that populates cloud_out and then updates
      ! bcf/qcl/qcf below.
      CALL crmml_ennuf(tqp_in_nn, cloud_out_nn)

      DO k = 1, 3*nn_nz
        cloud_out(k) = REAL(cloud_out_nn(1,k), KIND=real_umphys)
      END DO
      ...
      DO k = 1, nn_nz
        ...
        IF (test_kgo) THEN
          ...
          qcl(i,j,k) = lwc_out(k)
          qcf(i,j,k) = iwc_out(k)
          call log_event( 'Testing KGO: i,j,k,bcf, lwc, iwc', LOG_LEVEL_INFO )
...
!$OMP END PARALLEL
call log_event( 'End ENNUF_CLD', LOG_LEVEL_INFO )
